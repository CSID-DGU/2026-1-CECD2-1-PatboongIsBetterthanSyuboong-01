#!/usr/bin/env python3
"""
Dual Branch evaluation script.

Reads `outs_dual/dual_scores.json` (output of Dual Branch inference),
...
SCORES_PATH = Path("outs_dual/dual_scores.json")
OUTPUT_DIR = Path("eval_dual")
...
def compute_fixed_threshold(y_true: np.ndarray, scores: np.ndarray) -> float:
    """Validation-normal 기준 고정 threshold 계산 (p99.5 또는 mean+3σ)"""
...
def plot_confusion_matrix(metrics: Metrics, output_path: Path) -> None:
    ...
    ax.set_title(f"Confusion Matrix (Threshold={metrics.threshold:.6e})", 
...
def plot_score_distribution(y_true: np.ndarray, scores: np.ndarray, 
    ...
    ax.axvline(x=best_threshold, color="green", linestyle="--", linewidth=2, 
               label=f"Best Threshold ({best_threshold:.6e})")
...
def main() -> None:
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--f1-max':
        use_fixed_threshold = False
    
    y_true, scores = load_scores(SCORES_PATH)
    
    # 고정 threshold 또는 F1-max threshold 선택
    if use_fixed_threshold:
        threshold = compute_fixed_threshold(y_true, scores)
        best_metrics = confusion_at_threshold(y_true, scores, threshold)
        print(f"[INFO] 고정 threshold 사용 (validation-normal 기준): {threshold:.6e}")
    else:
        best_metrics = find_best_threshold(y_true, scores)
        print(f"[INFO] F1-max threshold 사용: {best_metrics.threshold:.6e}")
    ...
    with report_path.open("w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write(f"Dual Branch Evaluation Report - {timestamp}\n")
        f.write("=" * 80 + "\n\n")

        f.write(f"Samples: total={len(scores)}, normal={(y_true == 0).sum()}, abnormal={(y_true == 1).sum()}\n")
        if (y_true == 0).sum() == 0 or (y_true == 1).sum() == 0:
            f.write(f"[WARN] 클래스 불균형이 심합니다!\n")
        f.write("\n")
        
        threshold_type = "고정 threshold (validation-normal 기준)" if use_fixed_threshold else "Best Threshold (max F1)"
        f.write(f"{threshold_type}:\n")
        f.write(f"  threshold      : {best_metrics.threshold:.6e}\n")
        ...
        for count, left, right in zip(hist_counts, hist_edges[:-1], hist_edges[1:]):
            f.write(f"  [{left:.6e}, {right:.6e}): {count}\n")
        f.write("\n")

        f.write("Percentiles:\n")
        for p in [5, 25, 50, 75, 90, 95, 99]:
            f.write(f"  p{p:02d}: {np.percentile(scores, p):.6e}\n")
        f.write("\n")
    ...
    plot_confusion_matrix(best_metrics, timestamp_dir / f"confusion_matrix_{timestamp}.png")
    print(f"[INFO] Confusion matrix saved to {timestamp_dir / f'confusion_matrix_{timestamp}.png'}")
    
    plot_pr_curve(pr_recall, pr_precision, pr_auc, best_metrics.threshold, 
    ...
    plot_score_distribution(y_true, scores, best_metrics.threshold, 
